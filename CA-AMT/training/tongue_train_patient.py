...
from scr.datasets.patient_datasets  import TongueOnlyDataset
...
class TonguePatientNet(nn.Module):
    """
    不依赖 torchvision 的增强版单模态舌象模型
    比你之前的两层小 CNN 更强，但环境兼容性更好。
    """
...
def main():
    ...
    train_ds = TongueOnlyDataset(CSV_PATH, mode="train", resize=RESIZE)
    val_ds = TongueOnlyDataset(CSV_PATH, mode="val", resize=RESIZE)
    test_ds = TongueOnlyDataset(CSV_PATH, mode="test", resize=RESIZE)
    ...
    model = TonguePatientNet(num_classes=2).to(device)
    ...
    best_score = -1.0
    best_epoch = -1
    best_path = SAVE_DIR / "best_tongue_patient_model.pth"
    ...
    pred_csv = SAVE_DIR / "tongue_test_predictions.csv"
